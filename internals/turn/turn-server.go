...
func runTurn(turnStopChannel chan bool) {
	...
	publicIP := flag.String("public-ip", publicIp, "IP Address that TURN can be contacted by.")
	port := flag.Int("port", 3478, "Listening port.")
	users := flag.String("users", "\"user=pass,user=pass\"", "List of username and password (e.g. \"user=pass,user=pass\")")
	realm := flag.String("realm", "gengouconnect", "Realm ")
	threadNum := flag.Int("thread-num", 1, "Number of server threads (defaults to 1)")
	flag.Parse()

	fmt.Println("Starting the TURN Server")
